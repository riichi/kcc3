...
class BadgeUpdater:
    ...
    def update_badge(self):
        ...
        response = requests.post(self.badge.endpoint_url, json=body, auth=HmacAuth(self.badge.token))
    ...
    def __get_update_response(response) -> BadgeUpdateResponse:
        ...
        try:
            ...
            raise BadgeUpdateError(f"remote server sent an invalid response: {e}") from e

        return serializer.save()

    def __create_message_body(self):
        request = BadgeUpdateRequest(id=self.badge.id)
